...
string cSite::ToString() const
{
  ...
  // _snprintf_s(sz, TEMP_SIZE, "%06.3f%c, ", fabs(rad2deg(m_geo.m_Lat)), (LatNorth ? 'N' : 'S')); auskommentiert berge

   string strLoc = sz;

	//  _snprintf_s(sz, TEMP_SIZE, "%07.3f%c, ", fabs(rad2deg(m_geo.m_Lon)), (LonEast ? 'E' : 'W')); auskommentiert berge

   strLoc += sz;

	// _snprintf_s(sz, TEMP_SIZE, "%.1fm\n", (m_geo.m_Alt * 1000.0)); auskommentiert berge
